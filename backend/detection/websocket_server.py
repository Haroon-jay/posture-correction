    ...
    try:
        ...
        while True:
            ...
            if type == "img":
                ...
                count += 1
                # if result.pose_landmarks:
                #     print(result, result.pose_landmarks.landmark)
                #     features = extract_features_for_exercises(
                ...
                #     prediction = {"label": prediction, "confidence": 0.99}

                #     print(f"Prediction for {client_id}: {prediction}")

                # features = extract_features_for_exercises(data["landmarks"])
                # Append the frame to the user's buffer
                #

                # Check if the user's buffer has enough frames
                # if len(user_buffers[client_id]) == SEQUENCE_LENGTH:
                #     # Prepare the data for prediction
                #     input_sequence = np.array(
                #         user_buffers[client_id]
                ...
                #     #     input_sequence
                #     # )  # Call your model prediction function
                #     prediction = {"label": "bicep-curl", "confidence": 0.99}
                ...
                print(f"State for {client_id}: {img_len}")
